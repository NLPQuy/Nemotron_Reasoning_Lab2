# %% [markdown]
# # Batch-7 LoRA fusion (ZERO-TRAIN) — exp70
# Self-contained Kaggle CPU notebook: attach the 5x0.86 adapter dataset, Run All -> submission.zip.
# >>> EXP70 START
# EXP70 — B7-18 Core Space — LoRA-native core merge (bases from factors A,B).
# Mode: corespace(auto_pool(), substage=ties): shared U/V bases from factors, merge in small core, reconstruct.
# Note: ⭐ submit candidate; cheapest aligned merge (never builds dense [out,in]).
...
POOL = auto_pool()
corespace(POOL, substage="ties", density=0.7, out=OUT)
# >>> EXP70 END
...
deploy_check(OUT)
zip_submission(OUT)
print("exp70 done: /kaggle/working/submission.zip")
