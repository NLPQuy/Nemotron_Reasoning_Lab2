...
## EXP25 — Surface randomization (solver-invariant)  (Batch-3 Idea 6, DATA-AUG)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Upstream: nemotron-master/surface_instances.py (rand_* reordered-example copies, answer re-verified)  |  Corpus: EXP25_SURFACE_CORPUS
# Change: train on corpus with surface-randomized instances  |  Knob: EXP25_SURFACE_CORPUS=None  |  Rollback: EXP25_SURFACE_CORPUS=None + surface_instances.py --clear
...
# >>> EXP25 START
# Main EXP25 work is upstream: surface_instances.py appends rand_* (reordered examples,
# answer re-verified invariant). Set to the uploaded Kaggle dataset ROOT; override derives
# CORPUS_PATH=<root>/tokens and TRAIN_ORDER_PATH=<root>/logprobs/index.jsonl. None = baseline.
EXP25_SURFACE_CORPUS = None
# <<< EXP25 END
...
def run_training() -> None:
    ...
    # >>> EXP25 START
    if EXP25_SURFACE_CORPUS and IS_KAGGLE:
        CORPUS_PATH = os.path.join(EXP25_SURFACE_CORPUS, "tokens")
        TRAIN_ORDER_PATH = os.path.join(EXP25_SURFACE_CORPUS, "logprobs", "index.jsonl")
        print(f"EXP25 using surface-randomized snapshot root: {EXP25_SURFACE_CORPUS}")
    # <<< EXP25 END
