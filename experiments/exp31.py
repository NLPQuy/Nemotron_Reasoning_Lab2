...
# ============================================================
# EXP31 — Procedural cryptarithm generation + offline RFT  (Batch-4 Idea 2, P12, T2)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Upstream: nemotron-master/generators/cryptarithm_procedural.py (đọc store_types.py schema)
#   → generate 1,500 procedural problems → infer_slice temperature=0.5 n=10
#   → filter pass@10 >= 2% (falsify!) → add to corpus → pack → upload
# Falsify PRE-TRAINING: pass@10 < 2% trên 200 procedural problems → ABORT (không đủ signal)
# Change: CORPUS_PATH trỏ sang EXP31_CORPUS  |  Rollback: EXP31_CORPUS = None
# ============================================================
...
# >>> EXP31 START
EXP31_CORPUS = None  # set sau khi verify pass@10 >= 2% và upload corpus với RFT traces

# §5 falsification (đo accuracy model trên cryptarithm TRƯỚC khi train). Bật flag =>
# notebook chỉ chạy đo (greedy n=1 per solver-bucket) rồi DỪNG, KHÔNG train.
EXP31_RUN_S5_EVAL = False
# Dir chứa infer_slice.py + eval_slice.py + cryptarithm_eval_slice.jsonl + reasoning.py
# (dataset nemotron-master đã cập nhật). BASE/ADAPTER = path đã mount (offline).
EXP31_S5_NM_DIR = "/kaggle/input/<nemotron-master-dataset>/nemotron-master"
EXP31_S5_BASE = "/kaggle/input/<nemotron-base-dataset>"  # dir có config.json + modeling_*.py
EXP31_S5_ADAPTER = "/kaggle/input/<adapter-086-dataset>"  # dir có adapter_config.json
# <<< EXP31 END
...
def run_training() -> None:
    ...
    if EXP31_CORPUS:
        if IS_KAGGLE:
            CORPUS_PATH = os.path.join(EXP31_CORPUS, "tokens")
            TRAIN_ORDER_PATH = os.path.join(EXP31_CORPUS, "logprobs", "index.jsonl")
        print(f"EXP31: using corpus with procedural cryptarithm RFT traces from {EXP31_CORPUS}")
    # <<< EXP31 END
...
# %% [markdown] {"jupyter":{"outputs_hidden":false}}
# # EXP31 §5 — falsification: đo accuracy model trên cryptarithm (greedy n=1)
# Bật EXP31_RUN_S5_EVAL=True (config đầu file) để chạy chế độ NÀY thay vì train.
# Cần dataset nemotron-master (infer_slice.py vá + eval_slice.py + cryptarithm_eval_slice.jsonl)
# + BASE + ADAPTER 0.86 đã mount. Kết quả: accuracy per bucket concat/arith/unsolved.

# %% [code] {"jupyter":{"outputs_hidden":false}}
# >>> EXP31 START — §5 eval mode (inference-only, KHÔNG train)
def run_s5_eval() -> None:
    """Đo accuracy adapter 0.86 trên cryptarithm_eval_slice.jsonl, score per
    solver-bucket. Greedy n=1 (1 chuỗi/bài) -> KHÔNG OOM. KHÔNG train."""
    import os
    import subprocess
    ...
    nm = EXP31_S5_NM_DIR
    slice_path = os.path.join(nm, "cryptarithm_eval_slice.jsonl")
    assert os.path.isfile(slice_path), (
        f"Thiếu {slice_path}. Build local: uv run python3 build_cryptarithm_eval.py "
        "rồi thêm nemotron-master (đã cập nhật) làm dataset."
    )
    assert os.path.isfile(os.path.join(EXP31_S5_BASE, "config.json")), (
        f"EXP31_S5_BASE phải là dir model đã mount (có config.json): {EXP31_S5_BASE}"
    )
    assert os.path.isfile(os.path.join(EXP31_S5_ADAPTER, "adapter_config.json")), (
        f"EXP31_S5_ADAPTER phải là dir có adapter_config.json: {EXP31_S5_ADAPTER}"
    )

    # 1) Infer greedy n=1 (mặc định temperature=0, n_samples=1 -> schema {id, output}).
    subprocess.run(
        [
            "python",
            "infer_slice.py",
    ...
    # 2) Score per-bucket (category field = concat/arith/unsolved).
    subprocess.run(
        [
            "python",
            "eval_slice.py",
    ...
    print(
        "\n§5 xong. Đọc accuracy per bucket: concat&arith CAO (>=80%) => A/B bão hòa "
        "-> branch C; arith THẤP => build corpus A. (bảng đầy đủ ở experiments/exp31_s5_kaggle.md)"
    )
...
# %% [code] {"jupyter":{"outputs_hidden":false}}
# On Kaggle, trigger §5 eval (if flagged) or training directly after cells load.
...
if IS_KAGGLE:
    # >>> EXP31 START — flag chọn chế độ: đo §5 hay train
    if EXP31_RUN_S5_EVAL:
        run_s5_eval()
    else:
        run_training()
    # <<< EXP31 END