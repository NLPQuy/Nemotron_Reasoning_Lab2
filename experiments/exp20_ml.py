...
# ============================================================
# EXP20 — Procedural in-distribution instance scaling  (Batch-3 Idea 1, DATA-AUG)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Upstream: nemotron-master/generators/<cat>_gen.py + generate_instances.py (solver-labeled gen_* instances)  |  Corpus: EXP20_SCALED_CORPUS
# Change: train on corpus with extra solver-verified instances  |  Knob: EXP20_SCALED_CORPUS=None  |  Rollback: EXP20_SCALED_CORPUS=None + generate_instances.py --clear
# ============================================================
...
# >>> EXP20 START
# Main EXP20 work is upstream: generators/<cat>_gen.py + generate_instances.py append
# solver-verified gen_* instances, then reasoning.py + corpus.py + pack_kaggle_snapshot.py.
# Set to the uploaded Kaggle dataset ROOT; override derives CORPUS_PATH=<root>/tokens
# and TRAIN_ORDER_PATH=<root>/logprobs/index.jsonl. None = use the original baseline corpus.
EXP20_SCALED_CORPUS = None
# <<< EXP20 END
...
def run_training() -> None:
    ...
    if IS_KAGGLE:
        ...
        if not RESET_WEIGHTS:
            ...
            _adapter_zip = "/kaggle/input/no
            
            tebooks/huikang/tinker-submission-notebook/submission.zip"
    ...
    # >>> EXP20 START
    if EXP20_SCALED_CORPUS and IS_KAGGLE:
        CORPUS_PATH = os.path.join(EXP20_SCALED_CORPUS, "tokens")
        TRAIN_ORDER_PATH = os.path.join(EXP20_SCALED_CORPUS, "logprobs", "index.jsonl")
        print(f"EXP20 using scaled snapshot root: {EXP20_SCALED_CORPUS}")
    # <<< EXP20 END
