...
# ============================================================
# EXP22 — Masked auxiliary string-skill augmenters  (Batch-3 Idea 3, DATA-AUG)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Upstream: nemotron-master/augmenters/{reverse,count_substring,char_index,digit_extract}.py + augmentation.py  |  Corpus: EXP22_AUG_CORPUS
# Change: train on corpus regenerated with 4 new masked augmenters  |  Knob: EXP22_AUG_CORPUS=None  |  Rollback: EXP22_AUG_CORPUS=None + unregister augmenters
# ============================================================
...
# >>> EXP22 START
# Main EXP22 work is upstream in nemotron-master/augmenters/ + augmentation.py.
# Set to the uploaded Kaggle dataset ROOT built by pack_kaggle_snapshot.py; the
# override derives CORPUS_PATH=<root>/tokens and TRAIN_ORDER_PATH=<root>/logprobs/
# index.jsonl. None = use the original baseline corpus.
EXP22_AUG_CORPUS = None
# <<< EXP22 END
...
def run_training() -> None:
    ...
    # >>> EXP22 START
    if EXP22_AUG_CORPUS and IS_KAGGLE:
        CORPUS_PATH = os.path.join(EXP22_AUG_CORPUS, "tokens")
        TRAIN_ORDER_PATH = os.path.join(EXP22_AUG_CORPUS, "logprobs", "index.jsonl")
        print(f"EXP22 using augmenter-expanded snapshot root: {EXP22_AUG_CORPUS}")
    # <<< EXP22 END
