...
# ============================================================
# EXP14 — PiSSA LoRA initialization  (Batch-2 Idea 4)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: initialize LoRA with PiSSA and export vanilla-converted adapter  |  Knob: LORA_INIT=pissa_niter_4  |  Rollback: set LORA_INIT=gaussian
# ============================================================
...
PISSA_INIT_DIR = "/tmp/exp14_pissa_initial_adapter"
assert RESET_WEIGHTS, "EXP14 PiSSA init requires RESET_WEIGHTS=True"
# <<< EXP14 END
...
def run_training() -> None:
    ...
    # ── Wrap in LoRA ─────────────────────────────────────────────────
    # >>> EXP14 START
    # Unsloth's validate_loftq_config rejects init_lora_weights="pissa*"
    # (only True/False/gaussian/loftq/corda are whitelisted), but PEFT supports
    # PiSSA natively (data-free SVD of the base weight, no calibration set).
    # Patch the validator so the "pissa" string flows through to PEFT.
    if str(LORA_INIT).startswith("pissa"):
        import unsloth.models._utils as _uu_exp14
        import unsloth.models.vision as _uv_exp14

        _orig_validate_exp14 = _uu_exp14.validate_loftq_config

        def _validate_allow_pissa(
            loftq_config, lora_dropout, bias, init_lora_weights, model
        ):
            if isinstance(init_lora_weights, str) and init_lora_weights.startswith(
                "pissa"
            ):
                return None  # let PEFT handle PiSSA init
            return _orig_validate_exp14(
                loftq_config, lora_dropout, bias, init_lora_weights, model
            )

        _uu_exp14.validate_loftq_config = _validate_allow_pissa
        _uv_exp14.validate_loftq_config = _validate_allow_pissa
        print(f"EXP14 patched Unsloth validator to allow init={LORA_INIT}")
    # <<< EXP14 END
    model = FastLanguageModel.get_peft_model(
        ...
        # >>> EXP14 START
        # If Unsloth filters this kwarg, fallback is a manual PEFT LoraConfig
        # with init_lora_weights=LORA_INIT passed to peft.get_peft_model.
    ...
    print(f"EXP14 saved PiSSA initial adapter for conversion: {PISSA_INIT_DIR}")
    # <<< EXP14 END
