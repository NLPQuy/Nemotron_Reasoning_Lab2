...
#
# ============================================================
# EXP32 — AdaSTaR-inspired adaptive curriculum  (Batch-4 Idea 8)
# Paper: arXiv:2505.16322. The faithful AdaSTaR keeps a per-example win/lose
# min-heap that needs mid-training GENERATION+verification — infeasible inside
# this CCE-monkeypatched trainer. This is the TRACTABLE OFFLINE variant: use the
# running per-example cross-entropy (EMA) as a mastery proxy and, each step,
# sample the BATCH_SIZE *least-mastered* examples (highest EMA-CE) via Gumbel
# top-k. Same spirit ("train what you haven't learned") without generation.
#
# Knob: EXP32_USE_CURRICULUM (off => base linear walk, behavior-identical).
# Edits bracketed by START/END markers. Rollback: EXP32_USE_CURRICULUM=False.
# ============================================================
...
# >>> EXP32 START — AdaSTaR-inspired curriculum knobs
EXP32_USE_CURRICULUM = True  # False => base linear walk
EXP32_EMA_ALPHA = 0.3  # weight on the freshly measured CE in the EMA update
EXP32_PRIORITY_TEMP = 1.0  # Gumbel top-k temp: <1 greedier toward hard, >1 flatter
EXP32_UNSEEN_PRIORITY = 1e6  # priority for not-yet-trained examples (seen first)
# <<< EXP32 END
...
def run_training() -> None:
    ...
    # >>> EXP32 START — curriculum state + Gumbel-top-k least-mastered sampler
    if EXP32_USE_CURRICULUM:
        _prio_t = torch.full(
            (len(examples),), EXP32_UNSEEN_PRIORITY, dtype=torch.float32, device=device
        )  # per-example mastery proxy (EMA of avg completion CE)

        def _sample_batch_indices(k):
            # Gumbel top-k: weighted sampling WITHOUT replacement over log-priority,
            # so the least-mastered (highest EMA-CE) examples are picked each step.
            logp = torch.log(_prio_t.clamp_min(1e-9)) / max(1e-6, EXP32_PRIORITY_TEMP)
            g = -torch.log(-torch.log(torch.rand_like(_prio_t).clamp_min(1e-12)))
            return torch.topk(logp + g, min(k, _prio_t.numel())).indices.tolist()

        print(
            f"EXP32 curriculum ON: ema_alpha={EXP32_EMA_ALPHA}, "
            f"temp={EXP32_PRIORITY_TEMP} (sampling least-mastered each step)"
        )
    # <<< EXP32 END

    step = 0
    # >>> EXP32 START — while-loop drives base linear walk AND curriculum sampling
    _batch_cursor = 0
    while step < num_steps:
        _cur_ce: dict = {}
        if EXP32_USE_CURRICULUM:
            batch_indices = _sample_batch_indices(BATCH_SIZE)
        else:
            batch_indices = indices[_batch_cursor : _batch_cursor + BATCH_SIZE]
            _batch_cursor += BATCH_SIZE
        ...
        for mb_start in range(0, n, MICRO_BATCH_SIZE):
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                ...
                # >>> EXP32 START — capture per-example avg CE for the curriculum EMA
                if EXP32_USE_CURRICULUM:
                    _pw = padded_weights.sum(dim=-1).clamp_min(1.0)
                    _pe = (
                        (per_token_ce.detach() * padded_weights).sum(dim=-1) / _pw
                    ).tolist()
                    for _j, _ce_val in enumerate(_pe):
                        _cur_ce[batch_indices[mb_start + _j]] = float(_ce_val)
                # <<< EXP32 END
        ...
        # >>> EXP32 START — EMA-update mastery priorities for the trained examples
        if EXP32_USE_CURRICULUM and _cur_ce:
            for _gi, _ce_val in _cur_ce.items():
                _old = _prio_t[_gi].item()
                _prio_t[_gi] = (
                    _ce_val
                    if _old >= EXP32_UNSEEN_PRIORITY
                    else EXP32_EMA_ALPHA * _ce_val + (1 - EXP32_EMA_ALPHA) * _old
                )
        # <<< EXP32 END
