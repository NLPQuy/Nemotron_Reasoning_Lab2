...
# ============================================================
# EXP6 - LIMO/s1 difficulty + diversity curation  (Batch-1 Idea 6)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: category-balanced curation with a minimum completion length   |   Knob: CURATE=True   |   Rollback: set CURATE=False
# ============================================================
...
def run_training() -> None:
    ...
    if CURATE:
        ...
        def _cat(problem_id: str) -> str:
            return problem_id.split("-")[0] if "-" in problem_id else problem_id[:8]

        buckets: dict[str, list[tuple[int, dict]]] = defaultdict(list)
        for e in examples:
            comp_len = int(sum(e["weights"]))
            if comp_len < MIN_COMPLETION_LEN:
                continue
            buckets[_cat(e["problem_id"])].append((comp_len, e))

        curated: list[dict] = []
        for _cat_name, items in buckets.items():
            items.sort(key=lambda x: -x[0])
            curated.extend(e for _, e in items[:PER_CATEGORY_CAP])
        print(
            f"EXP6 curation: {len(examples)} → {len(curated)} "
            f"({len(buckets)} categories, cap={PER_CATEGORY_CAP})"
        )
        examples = curated
    # <<< EXP6 END
