...
# ============================================================
# EXP24 — Prompt paraphrase augmentation  (Batch-3 Idea 5, DATA-AUG)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Upstream: nemotron-master/paraphrase_instances.py (para_* reworded-prompt copies)  |  Corpus: EXP24_PARA_CORPUS
# Change: train on corpus with paraphrased-prompt instances  |  Knob: EXP24_PARA_CORPUS=None  |  Rollback: EXP24_PARA_CORPUS=None + paraphrase_instances.py --clear
# ============================================================
...
# >>> EXP24 START
# Main EXP24 work is upstream: paraphrase_instances.py appends para_* (reworded prompt,
# same examples/answer/trace). Set to the uploaded Kaggle dataset ROOT; override derives
# CORPUS_PATH=<root>/tokens and TRAIN_ORDER_PATH=<root>/logprobs/index.jsonl. None = baseline.
EXP24_PARA_CORPUS = None
# <<< EXP24 END
...
def run_training() -> None:
    ...
    # >>> EXP24 START
    if EXP24_PARA_CORPUS and IS_KAGGLE:
        CORPUS_PATH = os.path.join(EXP24_PARA_CORPUS, "tokens")
        TRAIN_ORDER_PATH = os.path.join(EXP24_PARA_CORPUS, "logprobs", "index.jsonl")
        print(f"EXP24 using paraphrase-augmented snapshot root: {EXP24_PARA_CORPUS}")
    # <<< EXP24 END
