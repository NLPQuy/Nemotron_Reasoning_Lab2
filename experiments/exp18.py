...
# ============================================================
# EXP18 — preference SimPO training  (Batch-2 Idea 8)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: optionally train on verified chosen/rejected pairs with SimPO  |  Knob: PAIRS_PATH=None  |  Rollback: set PAIRS_PATH=None
# ============================================================
...
# >>> EXP18 START
PAIRS_PATH = None  # optional offline-generated chosen/rejected pairs (jsonl)
EXP18_SYNTH_PAIRS = True  # if no PAIRS_PATH, build pairs in-notebook by corrupting the boxed answer
SIMPO_BETA = 2.0
SIMPO_GAMMA = 0.5
USE_PAIRS = (PAIRS_PATH is not None) or EXP18_SYNTH_PAIRS
# <<< EXP18 END
...
def run_training() -> None:
    ...
    if PAIRS_PATH is not None:
        ...
        def _pair_side_to_fields(side: dict):
            tokens = side["tokens"]
            mask = side["mask"]
            if len(tokens) > MAX_SEQ_LEN:
                tokens = tokens[:MAX_SEQ_LEN]
                mask = mask[:MAX_SEQ_LEN]
            return tokens[:-1], tokens[1:], [float(m) for m in mask[1:]]

        with open(PAIRS_PATH) as f:
            for line in f:
                rec = json.loads(line.strip())
                c_toks, c_tgts, c_wts = _pair_side_to_fields(rec["chosen"])
                r_toks, r_tgts, r_wts = _pair_side_to_fields(rec["rejected"])
                if not any(c_wts) or not any(r_wts):
                    continue
                pair_examples.append(
                    {
                        "problem_id": rec.get("problem_id", f"pair-{len(pair_examples)}"),
                        "tokens": c_toks,
                        "targets": c_tgts,
                        "weights": c_wts,
                        "rejected_tokens": r_toks,
                        "rejected_targets": r_tgts,
                        "rejected_weights": r_wts,
                    }
                )
        examples = pair_examples
        print(
            f"EXP18 loaded {len(examples)} SimPO pairs from {PAIRS_PATH}; "
            f"beta={SIMPO_BETA}, gamma={SIMPO_GAMMA}"
        )
    # <<< EXP18 END
    ...
    # >>> EXP18 START
    # Self-contained synthetic pairs: build a "rejected" twin of each example by
    # corrupting the value inside \boxed{...}. No sampling / verifier needed, so
    # the whole preference run fits in ONE notebook. (Skipped if PAIRS_PATH set.)
    if EXP18_SYNTH_PAIRS and PAIRS_PATH is None:
        import re as _re_exp18

        _BOX_RE = _re_exp18.compile(r"\\boxed\{([^}]*)\}")

        def _corrupt_val(v: str) -> str:
            v = v.strip()
            if _re_exp18.fullmatch(r"-?\d+", v):
                return str(int(v) + 1)
            if _re_exp18.fullmatch(r"-?\d*\.\d+", v):
                return f"{float(v) + 1.0}"
            if _re_exp18.fullmatch(r"[01]+", v):  # binary string
                return v[:-1] + ("0" if v[-1] == "1" else "1")
            return (v + "X") if v else "0"

        def _prompt_len(weights) -> int:
            n = 0
            for w in weights:
                if w > 0:
                    break
        ...
        synth = 0
        for e in examples:
            # reconstruct the full (unshifted) sequence + mask from stored shifts
            t_full = [e["tokens"][0]] + list(e["targets"])
            m_full = [0.0] + list(e["weights"])
            text = tokenizer.decode(t_full)
            mobj = None
            for mobj in _BOX_RE.finditer(text):
                pass  # keep the LAST boxed span
            if mobj is None:
                continue
            new_val = _corrupt_val(mobj.group(1))
            corrupted = text[: mobj.start(1)] + new_val + text[mobj.end(1) :]
            new_ids = tokenizer(corrupted, add_special_tokens=False)["input_ids"]
            plen = _prompt_len(m_full)
            if plen >= len(new_ids):
                continue
            new_mask = [0.0] * plen + [1.0] * (len(new_ids) - plen)
            e["rejected_tokens"] = new_ids[:-1]
            e["rejected_targets"] = new_ids[1:]
            e["rejected_weights"] = new_mask[1:]
            synth += 1
        # drop examples we could not pair (keeps batch building uniform)
        examples = [e for e in examples if "rejected_tokens" in e]
        print(
            f"EXP18 built {synth} synthetic SimPO pairs (corrupted boxed answer); "
            f"beta={SIMPO_BETA}, gamma={SIMPO_GAMMA}"
        )
    # <<< EXP18 END
    ...
    for batch_start in range(0, len(indices), BATCH_SIZE):
        ...
        # >>> EXP18 START
        if USE_PAIRS:
            batch_rejected_tokens = [e["rejected_tokens"] for e in batch]
            batch_rejected_targets = [e["rejected_targets"] for e in batch]
            batch_rejected_weights = [e["rejected_weights"] for e in batch]
        # <<< EXP18 END
        ...
        for mb_start in range(0, n, MICRO_BATCH_SIZE):
            ...
            # >>> EXP18 START
            if USE_PAIRS:
                mb_rej_toks = batch_rejected_tokens[mb_start:mb_end]
                mb_rej_tgts = batch_rejected_targets[mb_start:mb_end]
                mb_rej_wts = batch_rejected_weights[mb_start:mb_end]
            # <<< EXP18 END
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                ...
                # >>> EXP18 START
                if USE_PAIRS:
                    max_len_r = max(len(t) for t in mb_rej_toks)
                    padded_input_r = torch.zeros(
                        n_micro, max_len_r, dtype=torch.long, device=device
                    )
                    padded_targets_r = torch.zeros(
                        n_micro, max_len_r, dtype=torch.long, device=device
                    )
                    padded_weights_r = torch.zeros(
                        n_micro, max_len_r, dtype=torch.float32, device=device
                    )
                    attention_mask_r = torch.zeros(
                        n_micro, max_len_r, dtype=torch.long, device=device
                    )
                    for i in range(n_micro):
                        seq_len_r = len(mb_rej_toks[i])
                        padded_input_r[i, :seq_len_r] = torch.tensor(
                            mb_rej_toks[i], dtype=torch.long
                        )
                        padded_targets_r[i, :seq_len_r] = torch.tensor(
                            mb_rej_tgts[i], dtype=torch.long
                        )
                        padded_weights_r[i, :seq_len_r] = torch.tensor(
                            mb_rej_wts[i], dtype=torch.float32
                        )
                        attention_mask_r[i, :seq_len_r] = 1

                    model(
                        input_ids=padded_input_r,
                        attention_mask=attention_mask_r,
                        labels=padded_targets_r,
                        use_cache=False,
                    )
                    per_token_ce_r = model._cached_per_token_ce  # type: ignore[attr-defined]
                    weighted_loss_r = per_token_ce_r * padded_weights_r
                    weight_sum_r = padded_weights_r.sum()
                    loss_sum_r = weighted_loss_r.sum()
                    r_chosen = -(loss_sum_t / weight_sum_t.clamp(min=1.0))
                    r_rejected = -(loss_sum_r / weight_sum_r.clamp(min=1.0))
                    simpo_loss = -torch.nn.functional.logsigmoid(
                        SIMPO_BETA * (r_chosen - r_rejected) - SIMPO_GAMMA
                    )
                    loss = loss + simpo_loss
                    del per_token_ce_r, weighted_loss_r, simpo_loss
                # <<< EXP18 END
