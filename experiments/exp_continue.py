...
# ============================================================
# EXP_CONTINUE — continue-train the 0.86 adapter (2nd light SFT phase)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Idea: instead of fresh-retrain (exp20 fresh = 0.85 < 0.86), START from huikang's
#       0.86 adapter (RESET_WEIGHTS=False) and gently nudge it on our augmented corpus
#       at low LR / few steps. Isolates the marginal value of the augmentation ON TOP
#       of the proven baseline (exp20 confounded corpus change + losing huikang's recipe).
# Knobs: CONTINUE_CORPUS (augment snapshot root) | CONTINUE_ADAPTER_DIR (extracted 0.86 adapter dir)
# Rollback: RESET_WEIGHTS=True + LEARNING_RATE=2e-4 + NUM_STEPS=1000 (== fresh-train baseline)
# ============================================================
...
MAX_SEQ_LEN = 8192
# >>> EXP_CONTINUE START
NUM_STEPS = 300  # few steps: gentle 2nd phase, don't overwrite the 0.86 adapter
# <<< EXP_CONTINUE END
BATCH_SIZE = 32
MICRO_BATCH_SIZE = 4
# >>> EXP_CONTINUE START
LEARNING_RATE = 2e-5  # low LR (Plasticity-vs-Rigidity): nudge, not overwrite SFT-1
RESET_WEIGHTS = False  # continue from the pretrained 0.86 adapter, do not reset
# <<< EXP_CONTINUE END
IN_PROJ_ONLY = False
...
# >>> EXP_CONTINUE START
# MUST be True here: the loop consumes only the first NUM_STEPS*BATCH_SIZE examples in
# index order (no epoch wrap), and augment pids (gen_/para_/rand_) sort to the END of
# the corpus. Without shuffle, 300 steps never reach the augment → test would be a no-op.
SHUFFLE_DATASET = True
# <<< EXP_CONTINUE END

# >>> EXP_CONTINUE START
# Augment snapshot ROOT (built by pack_kaggle_snapshot.py; e.g. kaggle_snapshot_exp24).
# None = continue on the original baseline corpus. Derives CORPUS_PATH=<root>/tokens
# and TRAIN_ORDER_PATH=<root>/logprobs/index.jsonl.
CONTINUE_CORPUS = None
# Extracted 0.86 adapter dir (has adapter_config.json + adapter_model.safetensors),
# e.g. "/kaggle/input/datasets/mlinh776/adapter-0-86". None = use huikang's submission.zip path.
CONTINUE_ADAPTER_DIR = None
# <<< EXP_CONTINUE END
...
def run_training() -> None:
    ...
    if IS_KAGGLE:
        ...
        # >>> EXP_CONTINUE START — load 0.86 adapter from a dir if given, else huikang zip
        if not RESET_WEIGHTS and CONTINUE_ADAPTER_DIR:
            ADAPTER_SRC = CONTINUE_ADAPTER_DIR  # extracted adapter dir; no unzip needed
        elif not RESET_WEIGHTS:
    ...
    # >>> EXP_CONTINUE START
    if CONTINUE_CORPUS and IS_KAGGLE:
        CORPUS_PATH = os.path.join(CONTINUE_CORPUS, "tokens")
        TRAIN_ORDER_PATH = os.path.join(CONTINUE_CORPUS, "logprobs", "index.jsonl")
        print(f"EXP_CONTINUE continuing on augment snapshot: {CONTINUE_CORPUS}")
    print(f"EXP_CONTINUE: RESET_WEIGHTS={RESET_WEIGHTS}, LR={LEARNING_RATE}, steps={NUM_STEPS}")
    # <<< EXP_CONTINUE END
