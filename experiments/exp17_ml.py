...
# ============================================================
# EXP17 — LoRA seed-soup member  (Batch-2 Idea 7)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: make adapter seed controllable for soup members  |  Knob: SOUP_SEED=42  |  Rollback: set SOUP_SEED=42
# ============================================================
...
def run_training() -> None:
    ...
    # >>> EXP17 START
    random.seed(SOUP_SEED)
    np.random.seed(SOUP_SEED)
    torch.manual_seed(SOUP_SEED)
    torch.cuda.manual_seed_all(SOUP_SEED)
    print(f"EXP17 seed-soup member seed={SOUP_SEED}")
    # <<< EXP17 END
