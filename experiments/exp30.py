...
#
# ============================================================
# EXP30 — OXA exploration-aware SFT  (Batch-4 Idea 5)
# Paper: arXiv:2603.16206. Reweight each example by the CURRENT adapter's
# confidence on it: promote hard-but-correct traces (high avg CE = low logprob),
# suppress already-mastered ones (low avg CE = high confidence). OXA's per-example
# weight is a SCALAR -> baked as a multiplier on the per-token completion mask,
# which the base trainer already applies (`weights = [float(m) for m in mask]`),
# so NO training-loop surgery is needed.
#
# Pre-pass (requires RESET_WEIGHTS=False so we measure the 0.86 adapter): after
# loading the adapter, a no_grad forward over the corpus computes avg per-token CE
# per example -> OXA weight -> scale example["weights"] -> train normally.
#
# CAVEAT: corpus is verify-gated (all traces correct), so the paper's set-B
# "high-confidence INCORRECT" is empty; here set-B = "high-confidence correct"
# (already-learned) which we down-weight as an easy-example curriculum.
#
# Knob: EXP30_USE_OXA (off => identical to base trainer). Edits bracketed by
# START/END markers. Rollback: EXP30_USE_OXA=False.
# ============================================================
...
# >>> EXP30 START — OXA exploration-aware reweighting knobs
EXP30_USE_OXA = True  # False => identical to base trainer
# Threshold mode: "percentile" (robust, calibrates to the corpus) or "fixed".
EXP30_OXA_MODE = "percentile"
# percentile mode: promote the hardest top-p, suppress the easiest bottom-q (by avg CE).
EXP30_OXA_PROMOTE_PCT = 0.30
EXP30_OXA_SUPPRESS_PCT = 0.30
# fixed mode: avg per-token CE thresholds (CE = -logprob; paper logprob -0.5/-0.1).
EXP30_OXA_PROMOTE_CE = 0.50  # avg_ce above this => hard-correct => promote
EXP30_OXA_SUPPRESS_CE = 0.10  # avg_ce below this => mastered => suppress
EXP30_OXA_PROMOTE_WEIGHT = 2.0
EXP30_OXA_SUPPRESS_WEIGHT = 0.3
EXP30_OXA_MAX_EXAMPLES = 0  # >0 caps the logprob pre-pass for a smoke test (0 = all)
EXP30_OXA_MICRO_BATCH = 2  # pre-pass forward batch (small => lower peak VRAM on long seqs)
# <<< EXP30 END
...
def run_training() -> None:
    ...
    # >>> EXP30 START — OXA logprob pre-pass: measure the loaded adapter's avg CE
    # per example (no_grad), then scale each example's completion weights.
    if EXP30_USE_OXA:
        assert not RESET_WEIGHTS, (
            "EXP30_USE_OXA requires RESET_WEIGHTS=False (must measure the 0.86 adapter)."
        )
        oxa_pool = examples if EXP30_OXA_MAX_EXAMPLES <= 0 else examples[
            :EXP30_OXA_MAX_EXAMPLES
        ]
        # Memory: eval() stops Unsloth's gradient-offload hooks from accumulating
        # pinned CPU buffers across this long backward-free sweep (the cause of the
        # earlier "Kernel died" CPU-RAM OOM). Small batch + per-iter cache clear keep
        # peak VRAM flat. _OXA_MB caps the pre-pass micro-batch (long seqs x 4 spiked).
        _OXA_MB = min(MICRO_BATCH_SIZE, EXP30_OXA_MICRO_BATCH)
        print(
            f"EXP30 OXA pre-pass: scoring avg CE on {len(oxa_pool)} examples "
            f"(eval mode, micro_batch={_OXA_MB})..."
        )
        _was_training = model.training
        model.eval()
        avg_ces: list[float] = []
        _t0 = time.time()
        for _ei in range(0, len(oxa_pool), _OXA_MB):
            _mb = oxa_pool[_ei : _ei + _OXA_MB]
            _toks = [e["tokens"] for e in _mb]
            _tgts = [e["targets"] for e in _mb]
            _wts = [e["weights"] for e in _mb]
            _nm = len(_toks)
            _ml = max(len(t) for t in _toks)
            _pin = torch.zeros(_nm, _ml, dtype=torch.long, device=device)
            _ptg = torch.zeros(_nm, _ml, dtype=torch.long, device=device)
            _pwt = torch.zeros(_nm, _ml, dtype=torch.float32, device=device)
            _pat = torch.zeros(_nm, _ml, dtype=torch.long, device=device)
            for _i in range(_nm):
                _sl = len(_toks[_i])
                _pin[_i, :_sl] = torch.tensor(_toks[_i], dtype=torch.long)
                _ptg[_i, :_sl] = torch.tensor(_tgts[_i], dtype=torch.long)
                _pwt[_i, :_sl] = torch.tensor(_wts[_i], dtype=torch.float32)
                _pat[_i, :_sl] = 1
            with (
                torch.no_grad(),
                ...
                _ce = model._cached_per_token_ce.float()  # type: ignore[attr-defined]
                _num = (_ce * _pwt).sum(dim=-1)
                _den = _pwt.sum(dim=-1).clamp_min(1.0)
                avg_ces.extend(float(_v) for _v in (_num / _den).tolist())
            # Drop all refs + free the block so a long sweep cannot creep up.
            model._cached_per_token_ce = None  # type: ignore[attr-defined]
            del _pin, _ptg, _pwt, _pat, _ce, _num, _den
            if (_ei // _OXA_MB) % 50 == 0:
                torch.cuda.empty_cache()
                print(
                    f"  OXA {min(_ei + _OXA_MB, len(oxa_pool))}/{len(oxa_pool)} "
                    f"({time.time() - _t0:.0f}s, peak="
        ...
        if _was_training:
            model.train()
        torch.cuda.empty_cache()
        print(f"  OXA scoring done in {time.time() - _t0:.0f}s")

        _sorted = sorted(avg_ces)
        _n = len(_sorted)

        def _pct(p: float) -> float:
            return _sorted[min(_n - 1, max(0, int(p * _n)))]

        print(
            f"  avg_ce dist: min={_sorted[0]:.3f} p25={_pct(0.25):.3f} "
            f"median={_pct(0.5):.3f} p75={_pct(0.75):.3f} max={_sorted[-1]:.3f}"
        )
        if EXP30_OXA_MODE == "percentile":
            promote_thr = _pct(1.0 - EXP30_OXA_PROMOTE_PCT)  # hardest top-p
            suppress_thr = _pct(EXP30_OXA_SUPPRESS_PCT)  # easiest bottom-q
        else:
            promote_thr = EXP30_OXA_PROMOTE_CE
            suppress_thr = EXP30_OXA_SUPPRESS_CE
        print(
            f"  OXA mode={EXP30_OXA_MODE}: promote avg_ce>={promote_thr:.3f} "
            f"(w={EXP30_OXA_PROMOTE_WEIGHT}), suppress avg_ce<={suppress_thr:.3f} "
            f"(w={EXP30_OXA_SUPPRESS_WEIGHT})"
        )
        n_prom = n_supp = 0
        for _i, _ce_val in enumerate(avg_ces):
            if _ce_val >= promote_thr:
                _w = EXP30_OXA_PROMOTE_WEIGHT
                n_prom += 1
            elif _ce_val <= suppress_thr:
                _w = EXP30_OXA_SUPPRESS_WEIGHT
                n_supp += 1
            else:
                _w = 1.0
            if _w != 1.0:
                examples[_i]["weights"] = [m * _w for m in examples[_i]["weights"]]
        print(
            f"  OXA reweighted: {n_prom} promoted, {n_supp} suppressed, "
            f"{len(avg_ces) - n_prom - n_supp} unchanged (of {len(avg_ces)} scored)"
        )
        gc.collect()
        torch.cuda.empty_cache()
    # <<< EXP30 END
