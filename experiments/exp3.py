...
# ============================================================
# EXP3 - Difficulty-aware concise traces / anti-truncation  (Batch-1 Idea 3)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: drop overlong completion traces   |   Knob: DROP_OVERLONG=True   |   Rollback: set DROP_OVERLONG=False
# ============================================================
...
def run_training() -> None:
    ...
    if IS_KAGGLE:
        ...
        for sid in ordered_ids:
            ...
            if not any(mask):
                continue
            # >>> EXP3 START
            comp_len = sum(1 for m in mask if m)
    ...
    # >>> EXP3 START
    print(
        f"EXP3 anti-truncation: dropped {overlong_dropped} examples "
        f"with completion_len > {COMPLETION_BUDGET}"
    )
    # <<< EXP3 END
