...
# ============================================================
# EXP11 — DoRA weight-decomposed LoRA  (Batch-2 Idea 1)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: enable DoRA for trainable LoRA modules  |  Knob: USE_DORA=True  |  Rollback: set USE_DORA=False
# ============================================================
...
def run_training() -> None:
    ...
    model = FastLanguageModel.get_peft_model(
        ...
        # with use_dora=USE_DORA passed to peft.get_peft_model.
        use_dora=USE_DORA,
        # <<< EXP11 END
        bias="none",
    ...
    # >>> EXP11 START
    dora_params = [n for n, _ in model.named_parameters() if "lora_magnitude" in n]
    print(f"EXP11 DoRA lora_magnitude params: {dora_params[:3]}")
    # <<< EXP11 END
