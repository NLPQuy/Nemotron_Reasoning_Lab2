...
# ============================================================
# EXP28 — Drop off-distribution augmentation, add in-distribution mini-tasks  (Batch-4 Idea 4, P3, T1)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Upstream: nemotron-master/augmentation.py — comment-out 9 augmenter calls
#   + augmenters/mini_rule.py — 200–300 in-distribution mini-tasks per category
#   → uv run python3 corpus.py → verify size drops ~50% → pack → upload → set EXP28_CORPUS
# Falsify: eval_slice 50-step; nếu bit_manipulation/cipher drops > 2pp → restore augmentation
# Change: CORPUS_PATH trỏ sang EXP28_CORPUS  |  Rollback: EXP28_CORPUS = None + restore augmentation.py
# ============================================================
...
# >>> EXP28 START
EXP28_CORPUS = None  # e.g. "/kaggle/input/datasets/gracebml/nemotron-exp28-corpus"
# <<< EXP28 END
...
def run_training() -> None:
    ...
    if EXP28_CORPUS:
        if IS_KAGGLE:
            CORPUS_PATH = os.path.join(EXP28_CORPUS, "tokens")
            TRAIN_ORDER_PATH = os.path.join(EXP28_CORPUS, "logprobs", "index.jsonl")
        print(f"EXP28: using in-distribution-only corpus from {EXP28_CORPUS}")
    # <<< EXP28 END
