...
#
# ============================================================
# EXP33 — VCORE online variance-controlled CoT reweighting  (Batch-4 Idea 10)
# Paper: arXiv:2510.27462 (ACL 2026). Faithful port of the SINGLE-PROCESS
# anchor-window algorithm from enhance_cot/vcore (LLaMA-Factory + vendored
# transformers-4.52.4: loss/loss_utils.py::vcore_loss, sft/trainer.py
# ::_run_anchor_window_single_process). This is the REAL online method, NOT the
# static key-token heuristic (which the paper explicitly rejects).
#
# Per optimizer step (only on anchor steps): backup LoRA -> probe one random
# batch B' (theta' = theta - eps*grad) -> compute per-token pre_loss of the real
# batch under theta' -> restore theta -> real forward, reweight tokens by a
# masked softmax of (cur_loss - pre_loss)/(eps*tau) over the completion tokens,
# with variance control c=loss_uniform/loss_weighted (never amplify).
#
# Knob: EXP33_USE_VCORE (off => same path as base trainer). All EXP33 edits are
# bracketed by START/END markers below.
# Rollback: set EXP33_USE_VCORE=False (or run Continuer_Nemotron_Notebook.py).
# ============================================================
...
# >>> EXP33 START — VCORE online reweighting knobs
EXP33_USE_VCORE = True  # False => identical to base trainer
EXP33_VCORE_TEMPERATURE = 1.0  # tau: softens the per-token improvement softmax
EXP33_VCORE_EPSILON = 2e-5  # eps: probing LR for the branch step (= denom scale)
EXP33_VCORE_ANCHOR_STEPS = 1  # probe every N optimizer steps (1 = every step)
# <<< EXP33 END
...
def run_training() -> None:
    ...
    # >>> EXP33 START — VCORE helpers (no-op unless EXP33_USE_VCORE)
    _trainable = [p for p in model.parameters() if p.requires_grad]

    def _pad_micro(mb_toks, mb_tgts, mb_wts, device):
        """Mirror the base micro-batch padding (used only by the VCORE probe/pre passes)."""
        n_micro = len(mb_toks)
        ...
        for i in range(n_micro):
            ...
            attn[i, :sl] = 1
        return padded_input, padded_targets, padded_weights, attn

    def _vcore_weighted_loss(cur_ce, pre_ce, valid_f, eps, tau):
        """Faithful port of transformers-4.52.4 loss_utils.vcore_loss (the
        pre_loss branch): per-sequence masked softmax of the per-token loss
        *improvement* (cur - pre), length-rescaled, with variance control that
        never amplifies the loss above the uniform-CE baseline.

        cur_ce/pre_ce/valid_f: [B, T]; valid_f is the corpus completion mask
        (0 on prompt/pad). Returns a scalar (mean over valid tokens) so it slots
        into the existing `(loss / n_accum).backward()` accumulation.
        """
        valid_b = valid_f > 0
        cur_det = cur_ce.detach() * valid_f
        pre = pre_ce.to(cur_ce.device).detach() * valid_f
        scores = ((cur_det - pre) / (float(eps) * float(tau))).masked_fill(
            ~valid_b, float("-inf")
        )
        row_max = scores.max(dim=-1, keepdim=True).values
        exp_scores = torch.where(
            valid_b, (scores - row_max).exp(), torch.zeros_like(scores)
        )
        ...
        weights_token = weights.clamp_min(0.0) * valid_count  # sum/seq ~ #valid
        denom_tokens = valid_f.sum().clamp_min(1.0)
        loss_weighted = (weights_token * cur_ce * valid_f).sum() / denom_tokens
        loss_uniform = (cur_ce * valid_f).sum() / denom_tokens
        c = loss_uniform.detach() / loss_weighted.detach().clamp_min(1e-12)
        if c.item() <= 1.0:  # variance control: shrink only, never amplify
            loss_weighted = c * loss_weighted
        return loss_weighted

    _probe_rng = random.Random(1234)
    if EXP33_USE_VCORE:
        print(
            f"EXP33 VCORE online ON: tau={EXP33_VCORE_TEMPERATURE}, "
            f"eps={EXP33_VCORE_EPSILON}, anchor_steps={EXP33_VCORE_ANCHOR_STEPS}"
        )
    # <<< EXP33 END
    ...
    for batch_start in range(0, len(indices), BATCH_SIZE):
        ...
        # >>> EXP33 START — VCORE anchor-window probe (one branch step on a random
        # batch B', then per-token pre_loss of the real batch under theta').
        mb_pre: dict = {}
        vcore_anchor = EXP33_USE_VCORE and (step % EXP33_VCORE_ANCHOR_STEPS == 0)
        if vcore_anchor:
            _backup = [p.detach().clone() for p in _trainable]
            p_idx = _probe_rng.sample(
                range(len(examples)), min(BATCH_SIZE, len(examples))
            )
            p_batch = [examples[i] for i in p_idx]
            p_toks = [e["tokens"] for e in p_batch]
            p_tgts = [e["targets"] for e in p_batch]
            p_wts = [e["weights"] for e in p_batch]
            p_accum = math.ceil(len(p_batch) / MICRO_BATCH_SIZE)
            model.zero_grad(set_to_none=True)
            for ps in range(0, len(p_batch), MICRO_BATCH_SIZE):
                pe = min(ps + MICRO_BATCH_SIZE, len(p_batch))
                pin, ptg, pwt, pat = _pad_micro(
                    p_toks[ps:pe], p_tgts[ps:pe], p_wts[ps:pe], device
                )
                with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                    ...
                    pce = model._cached_per_token_ce  # type: ignore[attr-defined]
                    pwsum = pwt.sum()
                    ploss = (
                        (pce * pwt).sum() / pwsum
                        if pwsum > 0
                        else (pce * pwt).sum() * 0.0
                    )
                (ploss / p_accum).backward()
                del ploss, pce
            _tie_grads()  # keep tied MoE slices identical through the probe step
            with torch.no_grad():
                for p in _trainable:
                    if p.grad is not None:
                        p.data.add_(p.grad, alpha=-float(EXP33_VCORE_EPSILON))
            model.zero_grad(set_to_none=True)
            for ms in range(0, n, MICRO_BATCH_SIZE):
                me = min(ms + MICRO_BATCH_SIZE, n)
                rin, rtg, rwt, rat = _pad_micro(
                    batch_tokens[ms:me], batch_targets[ms:me], batch_weights[ms:me], device
                )
                with (
                    torch.no_grad(),
                    ...
                    mb_pre[ms // MICRO_BATCH_SIZE] = (
                        model._cached_per_token_ce.detach().float()  # type: ignore[attr-defined]
                    )
            with torch.no_grad():
                for p, b in zip(_trainable, _backup):
                    p.data.copy_(b)
            del _backup
            model.zero_grad(set_to_none=True)
        # <<< EXP33 END
        ...
        for mb_start in range(0, n, MICRO_BATCH_SIZE):
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                ...
                # >>> EXP33 START — VCORE-weighted loss on anchor steps (logged
                # loss_mean stays the plain masked-mean CE for comparability).
                _mb_idx = mb_start // MICRO_BATCH_SIZE
                if vcore_anchor and _mb_idx in mb_pre:
                    loss = _vcore_weighted_loss(
                        per_token_ce,
                        mb_pre[_mb_idx],
                        padded_weights,
                        EXP33_VCORE_EPSILON,
                        EXP33_VCORE_TEMPERATURE,
                    )
                    loss_sum_t = (per_token_ce.detach() * padded_weights).sum()
                else:
