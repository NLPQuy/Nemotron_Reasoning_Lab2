...
# ============================================================
# EXP8 - Hot-expert untying of MOE_TIE_WEIGHTS  (Batch-1 Idea 8)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: keep cold experts tied while hot experts keep independent LoRA grads   |   Knob: UNTIE_HOT_EXPERTS=True   |   Rollback: set UNTIE_HOT_EXPERTS=False
# ============================================================
...
# >>> EXP8 START
UNTIE_HOT_EXPERTS = True
NUM_HOT = 8
HOT_EXPERT_PROFILE_BATCHES = 2
# <<< EXP8 END
...
def run_training() -> None:
    ...
    # >>> EXP8 START
    hot_idx: set[int] = set()
    if UNTIE_HOT_EXPERTS and MOE_TIE_WEIGHTS:
        expert_counts: torch.Tensor | None = None
        hooks = []

        def _router_hook(_module, _inputs, output):
            ...
            counts = torch.bincount(chosen.cpu(), minlength=flat.shape[-1])
            expert_counts = counts if expert_counts is None else expert_counts + counts

        for module_name, module in model.named_modules():
            if module_name.endswith("mixer.gate"):
                hooks.append(module.register_forward_hook(_router_hook))

        if hooks:
            profile_n = min(len(examples), HOT_EXPERT_PROFILE_BATCHES * MICRO_BATCH_SIZE)
            profile_batch = examples[:profile_n]
            if profile_batch:
                prof_device = next(model.parameters()).device
                max_len = max(len(e["tokens"]) for e in profile_batch)
                padded_input = torch.zeros(
                    len(profile_batch), max_len, dtype=torch.long, device=prof_device
                )
                attention_mask = torch.zeros(
                    len(profile_batch), max_len, dtype=torch.long, device=prof_device
                )
                for i, e in enumerate(profile_batch):
                    seq_len = len(e["tokens"])
                    padded_input[i, :seq_len] = torch.tensor(
                        e["tokens"], dtype=torch.long, device=prof_device
                    )
        ...
        if expert_counts is not None and expert_counts.numel() > 0:
            hot_idx = set(
                int(i)
                for i in torch.topk(expert_counts, k=min(NUM_HOT, expert_counts.numel())).indices
            )
            print(f"EXP8 hot experts from router profile: {sorted(hot_idx)}")
        else:
            hot_idx = set(range(NUM_HOT))
            print(f"EXP8 hot expert profile unavailable; fallback hot_idx={sorted(hot_idx)}")
    # <<< EXP8 END
    ...
    if MOE_TIE_WEIGHTS:
        ...
        def _tie_param_init() -> None:
            """Make cold expert slices identical; hot slices stay independent."""
            with torch.no_grad():
                for p in moe_tied_params:
                    ...
                    if UNTIE_HOT_EXPERTS:
                        ...
                        if not cold:
                            continue
                        mean = p.data[cold].mean(dim=0, keepdim=True)
                        p.data[cold] = mean.expand(len(cold), *p.data.shape[1:])
                    else:
            ...
            with torch.no_grad():
                for p in moe_tied_params:
                    ...
                    if UNTIE_HOT_EXPERTS:
                        ...
                        if not cold:
                            continue
                        grad_sum = p.grad[cold].sum(dim=0, keepdim=True)
                        p.grad[cold] = grad_sum.expand(len(cold), *p.grad.shape[1:])
                    else:
