...
# ============================================================
# EXP10 - Simulated-annealing difficulty curriculum  (Batch-1 Idea 10)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: sample batches by an easy-to-hard annealed difficulty distribution   |   Knob: ANNEAL_CURRICULUM=True   |   Rollback: set ANNEAL_CURRICULUM=False
# ============================================================
...
def run_training() -> None:
    ...
    # >>> EXP10 START
    if ANNEAL_CURRICULUM:
        import numpy as np

        np_rng = np.random.default_rng(0)
        diff = np.array([sum(e["weights"]) for e in examples], dtype=np.float64)
        diff = (diff - diff.mean()) / (diff.std() + 1e-6)

        def _sample_batch(cur_step: int, batch_size: int) -> list[int]:
            progress = cur_step / max(1, num_steps)
            temp = T0 + (T1 - T0) * progress
            logits = -diff / max(temp, 1e-3)
        ...
        print(f"EXP10 annealed curriculum: T0={T0}, T1={T1}, seed=0")
    # <<< EXP10 END
    ...
    # >>> EXP10 START
    max_steps = NUM_STEPS if ANNEAL_CURRICULUM else len(examples) // BATCH_SIZE
    # <<< EXP10 END
    num_steps = NUM_STEPS
    ...
    step = 0
    # >>> EXP10 START
    loop_span = num_steps * BATCH_SIZE if ANNEAL_CURRICULUM else len(indices)
    # <<< EXP10 END
