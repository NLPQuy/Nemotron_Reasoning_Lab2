...
# ============================================================
# EXP16 — anchored-SFT KL regularization  (Batch-2 Idea 6)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: add KL-to-base anchor on completion tokens  |  Knob: KL_ANCHOR_BETA=0.1  |  Rollback: set KL_ANCHOR_BETA=0
# ============================================================
...
# >>> EXP16 START
KL_ANCHOR_BETA = 0.1
KL_MAX_TOKENS = 2048  # cap masked positions per micro-batch for KL (memory guard)
KL_EVERY = 1  # compute KL every N micro-batches (raise to cut compute)
# <<< EXP16 END
    ...
    for batch_start in range(0, len(indices), BATCH_SIZE):
        ...
        for mb_start in range(0, n, MICRO_BATCH_SIZE):
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                ...
                # >>> EXP16 START
                _mb_idx_kl = mb_start // MICRO_BATCH_SIZE
                if (
                    KL_ANCHOR_BETA > 0
                    and weight_sum_t > 0
                    and (_mb_idx_kl % KL_EVERY == 0)
                ):
                    # KL anchor on completion tokens only. The CCE path avoids
                    # materializing logits; OOM came from running lm_head over the
                    # full (B, L, vocab). Fix: gather hidden states at masked
                    # positions BEFORE lm_head, so logits are only (n_sel, vocab),
                    # and subsample to KL_MAX_TOKENS.
                    m = padded_weights > 0
                    idx = m.nonzero(as_tuple=False)
                    if idx.shape[0] > KL_MAX_TOKENS:
                        sel = torch.randperm(idx.shape[0], device=idx.device)[
                            :KL_MAX_TOKENS
                        ]
                        idx = idx[sel]
                    rows, cols = idx[:, 0], idx[:, 1]
                    with model.disable_adapter(), torch.no_grad():
                        base_hidden = _base.backbone(
                            input_ids=padded_input,
                            attention_mask=attention_mask,
                            use_cache=False,
                        )[0]
                        base_logits = _base.lm_head(base_hidden[rows, cols])
                    pol_hidden = _base.backbone(
                        input_ids=padded_input,
                        attention_mask=attention_mask,
                        use_cache=False,
                    )[0]
                    pol_logits = _base.lm_head(pol_hidden[rows, cols])
                    ...
                    del base_hidden, base_logits, pol_hidden, pol_logits, kl
                # <<< EXP16 END
