...
# ============================================================
# EXP1 - Format-verified clean labels + truncation-robust completions  (Batch-1 Idea 1)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: keep boxed tail on truncation and drop examples without a valid box   |   Knob: TRUNCATION_KEEP_BOXED_TAIL=True   |   Rollback: set TRUNCATION_KEEP_BOXED_TAIL=False
# ============================================================
...
def run_training() -> None:
    ...
    _verify_tok = AutoTokenizer.from_pretrained(MODEL_PATH, trust_remote_code=True)
    _BOX_RE = re.compile(r"\\boxed\{.*\}", re.DOTALL)
    truncated_examples = 0

    def _truncate_with_boxed_tail(
        token_ids: list[int], mask_values: list[int]
    ) -> tuple[list[int], list[int], bool]:
        if len(token_ids) <= MAX_SEQ_LEN:
            return token_ids, mask_values, False
        if not TRUNCATION_KEEP_BOXED_TAIL:
            return token_ids[:MAX_SEQ_LEN], mask_values[:MAX_SEQ_LEN], True
        head_len = max(1, MAX_SEQ_LEN - BOXED_TAIL_TOKENS)
        return (
            token_ids[:head_len] + token_ids[-BOXED_TAIL_TOKENS:],
            mask_values[:head_len] + mask_values[-BOXED_TAIL_TOKENS:],
            True,
        )

    def _has_valid_box(token_ids: list[int]) -> bool:
        text = _verify_tok.decode(token_ids)
    ...
    if IS_KAGGLE:
        ...
        for sid in ordered_ids:
            ...
            if len(tokens) > MAX_SEQ_LEN:
                tokens, mask, _was_truncated = _truncate_with_boxed_tail(tokens, mask)
                truncated_examples += int(_was_truncated)
            # <<< EXP1 END
            if not any(mask):
                continue
            examples.append(
        ...
        with open(CORPUS_PATH) as f:
            for line in f:
                ...
                # >>> EXP1 START
                if len(tokens) > MAX_SEQ_LEN:
                    tokens, mask, _was_truncated = _truncate_with_boxed_tail(
                        tokens, mask
                    )
                    truncated_examples += int(_was_truncated)
                # <<< EXP1 END
    ...
    print(
        f"EXP1 format-verify: truncated {truncated_examples} examples, "
        f"dropped {before - len(examples)} bad-box examples"
    )
    # <<< EXP1 END
