...
# ============================================================
# EXP2 - Up-weight boxed / critical answer tokens  (Batch-1 Idea 2)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: multiply loss weights on the boxed answer span   |   Knob: ANSWER_TOKEN_WEIGHT=2.0   |   Rollback: set ANSWER_TOKEN_WEIGHT=1.0
# ============================================================
...
ANSWER_FALLBACK_TAIL_TOKENS = 64
# <<< EXP2 END
...
def run_training() -> None:
    ...
    _weight_tok = AutoTokenizer.from_pretrained(MODEL_PATH, trust_remote_code=True)

    def _boxed_token_range(token_ids: list[int]) -> tuple[int, int] | None:
        text = _weight_tok.decode(token_ids)
        box_start = text.rfind("\\boxed{")
        if box_start < 0:
            return None
        marker_start = text.rfind("</think>", 0, box_start)
        lo = marker_start if marker_start >= 0 else box_start
        hi = text.find("}", box_start)
        ...
        positive = [i for i, _ in enumerate(token_ids)]
        if not positive:
            return None
        return max(0, len(token_ids) - ANSWER_FALLBACK_TAIL_TOKENS), len(token_ids) - 1
    ...
    if ANSWER_TOKEN_WEIGHT != 1.0:
        ...
        for e in examples:
            ...
            a, b = rng
            touched = 0
            for k in range(max(0, a), min(len(e["weights"]), b + 1)):
                if e["weights"][k] > 0:
                    e["weights"][k] = max(
                        1.0, float(e["weights"][k]) * ANSWER_TOKEN_WEIGHT
                    )
                    touched += 1
            weighted_tokens += touched
            weighted_examples += int(touched > 0)
        print(
            f"EXP2 boxed weighting: weighted {weighted_tokens} tokens "
            f"across {weighted_examples} examples at lambda={ANSWER_TOKEN_WEIGHT}"
        )
    # <<< EXP2 END
