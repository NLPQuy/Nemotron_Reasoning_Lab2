...
# ============================================================
# EXP27 — Extend bit_manipulation solver: rotation/shift/majority  (Batch-4 Idea 1, P3, T1)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Upstream: nemotron-master/reasoners/bit_manipulation.py — thêm LEFT_ROTATE_k,
#   RIGHT_ROTATE_k (k=1..7), LEFT_SHIFT_k, RIGHT_SHIFT_k, MAJORITY operators
#   → uv run python3 reasoning.py → count mới rule_found >= 30 → uv run python3 corpus.py
#   → pack_kaggle_snapshot.py --tag exp27 → upload → set EXP27_CORPUS below
# Change: CORPUS_PATH trỏ sang EXP27_CORPUS snapshot  |  Rollback: EXP27_CORPUS = None
# ============================================================
...
# >>> EXP27 START
# Set to Kaggle dataset root sau khi upload snapshot (pack_kaggle_snapshot.py --tag exp27).
# None = dùng baseline corpus của KAGGLE_DATASET.
EXP27_CORPUS = None  # e.g. "/kaggle/input/datasets/gracebml/nemotron-exp27-corpus"
# <<< EXP27 END
...
def run_training() -> None:
    ...
    # >>> EXP27 START — override CORPUS_PATH khi EXP27_CORPUS được set
    if EXP27_CORPUS:
        if IS_KAGGLE:
            CORPUS_PATH = os.path.join(EXP27_CORPUS, "tokens")
            TRAIN_ORDER_PATH = os.path.join(EXP27_CORPUS, "logprobs", "index.jsonl")
        print(f"EXP27: using solver-extended corpus from {EXP27_CORPUS}")
    # <<< EXP27 END
