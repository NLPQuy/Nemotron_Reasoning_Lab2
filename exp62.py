# %% [markdown]
# # Batch-7 LoRA fusion (ZERO-TRAIN) — exp62
# Self-contained Kaggle CPU notebook: attach the 5x0.86 adapter dataset, Run All -> submission.zip.
# >>> EXP62 START
# EXP62 — B7-3 WiSE-FT — pairwise interpolation of two adapters.
# Mode: wiseft(ALPHA, A, B) = soup([A,B], [1-ALPHA, ALPHA]) over the two most distant discovered adapters.
# Note: Sweep ALPHA in {0.3,0.5,0.7}; reorder POOL / slice for a specific pair.
...
POOL = auto_pool()
PAIR = POOL[:2]  # interpolate the first two discovered (reorder POOL / re-slice for another pair)
ALPHA = 0.5  # sweep {0.3,0.5,0.7} across Kaggle runs
wiseft(ALPHA, PAIR[0], PAIR[1], out=OUT)
# >>> EXP62 END
...
deploy_check(OUT)
zip_submission(OUT)
print("exp62 done: /kaggle/working/submission.zip")
