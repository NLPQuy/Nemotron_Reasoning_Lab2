# %% [markdown]
# # Batch-7 LoRA fusion (ZERO-TRAIN) — exp68
# Self-contained Kaggle CPU notebook: attach the 5x0.86 adapter dataset, Run All -> submission.zip.
# >>> EXP68 START
# EXP68 — B7-8 small-alpha soup — soup 0.86 pool with extra 0.84 adapters at low weight.
# Mode: wsoup: blend POOL5 (0.86) with separately-uploaded 0.84 adapters down-weighted.
# Note: REQUIRES extra 0.84 adapters uploaded to the dataset; run last, after exp58>=0.86.
...
POOL = find_pool(["exp32-0-86", "exp40-0-86", "exp43-0-86", "exp49-0-86", "exp53-0-86"])
# Upload 0.84 adapters into the dataset and list their folder names here:
POOL_084 = []  # e.g. ["exp21-0-84", "expXX-0-84"]
assert POOL_084, "exp68 needs >=1 extra 0.84 adapter uploaded to the dataset"
extra = find_pool(POOL_084)
adirs = POOL + extra
w = [1.0] * len(POOL) + [0.25] * len(extra)        # down-weight the 0.84 ingredients
w = [x / sum(w) for x in w]
soup(adirs, weights=w, out=OUT)
# >>> EXP68 END
...
deploy_check(OUT)
zip_submission(OUT)
print("exp68 done: /kaggle/working/submission.zip")
