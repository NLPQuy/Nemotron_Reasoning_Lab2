...
# ============================================================
# EXP5 - Reasoning-critical target-module and rank realloc  (Batch-1 Idea 5)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: focus LoRA targets on MLP/expert, mixer, o_proj, and lm_head   |   Knob: TARGET_MODULES focused list   |   Rollback: restore original TARGET_MODULES
# ============================================================
...
def run_training() -> None:
    ...
    # >>> EXP5 START
    target_hits = {target: 0 for target in TARGET_MODULES}
    for module_name, _module in model.named_modules():
        short_name = module_name.rsplit(".", 1)[-1]
        if short_name in target_hits:
            target_hits[short_name] += 1
    print(f"EXP5 target module audit: {target_hits}")
    # <<< EXP5 END
