...
# >>> EXP47 START
# EXP47 — Quality-gate corpus + anchored-L2 (Batch-5 D4 + D9)
# Ref: plan-batch-5.md §D4/exp47 | Knob: ANCHOR_LAMBDA=1e-3 | Rollback: revert corpus to baseline
# >>> EXP47 END
...
MAX_SEQ_LEN = 8192
# >>> EXP47_CONT START   (continue-train từ 0.86 — luật 1+3 batch-5)
NUM_EPOCHS = 1.0
NUM_STEPS = None
...
def run_training() -> None:
    ...
    if IS_KAGGLE:
        ...
        # >>> EXP47_CORPUS START  (D4: quality-gate variant — uses JSONL reader)
        CORPUS_PATH = "/kaggle/input/datasets/ngoczhu/nemotron-corpus-qualitygate/corpus_qualitygate.jsonl"
        # >>> EXP47_CORPUS END
        TRAIN_ORDER_PATH = "/kaggle/input/datasets/huikang/huikang-nemotron-repository-snapshot/nemotron-master/training/sft/04-08-16-14/logprobs/index.jsonl"  # unused when JSONL
    ...
    # >>> EXP47_FULLSIZE_GUARD START
    # D4 quality-gate XOÁ HỢP LỆ trace hypo+unknown (giữ rule_found+aug): ~16830 dòng / ~46.2M token.
    # Guard cho phép giảm tới mức này NHƯNG chặn subset sâu hơn (lỗi batch-4).
    if IS_KAGGLE:
        _exp_rows, _exp_tok = 16830, 46_220_000
        assert abs(len(examples) - _exp_rows) <= 0.05 * _exp_rows, (
            f"FULL-SIZE GUARD: {len(examples)} dòng lệch >5% so với {_exp_rows} (qualitygate) — kiểm filter"
        )
        assert total_tokens >= 0.90 * _exp_tok, (
            f"FULL-SIZE GUARD: {total_tokens:,} token < 90% của {_exp_tok:,} — corpus thiếu"
        )
        print(f"FULL-SIZE GUARD ok: {len(examples)} dòng / {total_tokens:,} token")
    # >>> EXP47_FULLSIZE_GUARD END
    ...
    # >>> EXP47 START  (anchor from EXP42)
    ANCHOR_LAMBDA = 1e-3
    theta_ref = {n: p.detach().float().clone() for n, p in model.named_parameters() if p.requires_grad}
    print(f"EXP47: snapshot theta_ref ({len(theta_ref)} params, ANCHOR_LAMBDA={ANCHOR_LAMBDA})")
    # >>> EXP47 END
    ...
    for batch_start in range(0, len(indices), BATCH_SIZE):
        ...
        # >>> EXP47 START  (anchor gradient from EXP42)
        with torch.no_grad():
