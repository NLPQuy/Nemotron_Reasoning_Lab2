...
# ============================================================
# EXP9 - Spaced-repetition data scheduling  (Batch-1 Idea 9)
# Base: Continuer_Nemotron_Notebook.py (unmodified except marked blocks)
# Change: replace one-pass order with loss-EMA due scheduling   |   Knob: SPACED_REPETITION=True   |   Rollback: set SPACED_REPETITION=False
# ============================================================
...
def run_training() -> None:
    ...
    if SPACED_REPETITION:
        ...
        ema_loss = {i: 1.0 for i in range(len(examples))}
        due = [(0.0, i) for i in range(len(examples))]
        heapq.heapify(due)

        def _next_batch(batch_size: int) -> list[int]:
            picked: list[int] = []
            while due and len(picked) < batch_size:
                picked.append(heapq.heappop(due)[1])
            return picked

        def _reschedule(example_idx: int, cur_step: int, correctish: bool) -> None:
            interval = INTERVAL_BASE ** max(
                0, min(8, -int(math.log(ema_loss[example_idx] + 1e-6)))
            )
            heapq.heappush(
                due, (cur_step + (interval if correctish else 1), example_idx)
            )

        print(
            f"EXP9 spaced repetition: review_every={REVIEW_EVERY}, "
            f"interval_base={INTERVAL_BASE}"
        )
    # <<< EXP9 END
    ...
    # >>> EXP9 START
    max_steps = NUM_STEPS if SPACED_REPETITION else len(examples) // BATCH_SIZE
    # <<< EXP9 END
    num_steps = NUM_STEPS
    ...
    step = 0
    # >>> EXP9 START
    loop_span = num_steps * BATCH_SIZE if SPACED_REPETITION else len(indices)
    # <<< EXP9 END
    for batch_start in range(0, loop_span, BATCH_SIZE):
        if step >= num_steps:
            break
        # >>> EXP9 START
        batch_indices = (
            _next_batch(BATCH_SIZE)
            if SPACED_REPETITION
            else indices[batch_start : batch_start + BATCH_SIZE]
        )
        ...
        # >>> EXP9 START
        batch_loss_by_index: dict[int, float] = {}
        # <<< EXP9 END
        ...
        for mb_start in range(0, n, MICRO_BATCH_SIZE):
            ...
            # >>> EXP9 START
            mb_ids = batch_indices[mb_start:mb_end]
            # <<< EXP9 END
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                ...
                # >>> EXP9 START
                if SPACED_REPETITION:
                    seq_loss_sum = weighted_loss.detach().sum(dim=1)
                    seq_weight_sum = padded_weights.detach().sum(dim=1).clamp_min(1.0)
                    seq_losses = (seq_loss_sum / seq_weight_sum).cpu().tolist()
                    for example_idx, seq_loss in zip(mb_ids, seq_losses, strict=False):
                        batch_loss_by_index[example_idx] = float(seq_loss)
                # <<< EXP9 END
                loss = (
        ...
        # >>> EXP9 START
        if SPACED_REPETITION:
            for example_idx, seq_loss in batch_loss_by_index.items():
                ema_loss[example_idx] = 0.9 * ema_loss[example_idx] + 0.1 * seq_loss
                _reschedule(
                    example_idx,
                    step + 1,
                    correctish=ema_loss[example_idx] <= max(loss_mean, 1e-6),
                )
        # <<< EXP9 END
        step += 1
        _log(
