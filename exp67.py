# %% [markdown]
# # Batch-7 LoRA fusion (ZERO-TRAIN) — exp67
# Self-contained Kaggle CPU notebook: attach the 5x0.86 adapter dataset, Run All -> submission.zip.
# >>> EXP67 START
# EXP67 — B7-2 subset-select — greedy diversity subset of the pool by diag, then soup.
# Mode: subset_select(auto_pool()) -> diverse subset -> soup. No accuracy signal; diag-cosine driven.
# Note: Yields one subset; keeps only mutually-distant adapters (cos < thresh).
...
POOL = auto_pool()
SUBSET = subset_select(POOL, thresh=0.95)
soup(SUBSET, out=OUT)
# >>> EXP67 END
...
deploy_check(OUT)
zip_submission(OUT)
print("exp67 done: /kaggle/working/submission.zip")
