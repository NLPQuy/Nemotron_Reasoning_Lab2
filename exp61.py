# %% [markdown]
# # Batch-7 LoRA fusion (ZERO-TRAIN) — exp61
# Self-contained Kaggle CPU notebook: attach the 5x0.86 adapter dataset, Run All -> submission.zip.
# >>> EXP61 START
# EXP61 — B7-12 SeedLoRA — two-stage robust/conflict + subspace residual fusion.
# Mode: seedlora(auto_pool(), sigma=0.9, substage=ties): Stage1 element robust/conflict, Stage2 subspace.
# Note: ⭐⭐ strongest evidence for same-task LoRA-merge>best-single; eats the pool diversity.
...
POOL = auto_pool()
seedlora(POOL, sigma=0.9, substage="ties", out=OUT)
# >>> EXP61 END
...
deploy_check(OUT)
zip_submission(OUT)
print("exp61 done: /kaggle/working/submission.zip")
