#!/usr/bin/env python3
"""
Generate one shared rollout file with vLLM.

Output rollouts.jsonl feeds downstream offline generators. It keeps every
sampled group and stores prompt/completion ids separately so corpus builders can
call offline.common.tokenize_format.make_example_from_ids.
"""
...
def main():
    ...
    lora_request = None
    if args.adapter_path and os.path.isfile(
        os.path.join(args.adapter_path, "adapter_config.json")
    ):
        try:
            from vllm.lora.request import LoRARequest

            lora_request = LoRARequest("offline_adapter", 1, args.adapter_path)
        ...
        with open(args.output, mode) as fout:
            for start in range(0, len(rows), batch_size):
                ...
                n_written, n_mixed = process_batch(
                    batch, tokenizer, llm, lora_request, args, fout
                )
                ...
                mixed_rate = total_mixed_groups / done if done > 0 else 0
                print(
                    f"[{done}/{len(rows)}] mixed_groups={total_mixed_groups} "
