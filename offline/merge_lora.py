#!/usr/bin/env python3
"""Batch-7 LoRA merge engine (product-space SVD, L0.3/L0.4). CPU-only, zero-train.

This is the source-of-truth core for dev + local testing. Each Kaggle expN.py
(exp58..exp70) INLINES these same core functions verbatim plus a `# >>> EXP<N>`
block holding the POOL / mode / params (the repo's `offline/` is not present on
Kaggle). Keep the two in sync.

HARD RULE (L0.3): merge on the PRODUCT delta W = B@A, then SVD-truncate to rank 32.
NEVER average lora_A / lora_B factors separately — B@A is invariant to
(B*g, g^-1*A), so factor averaging injects cross-terms B_i A_j that destroy the
delta. `soup_adapters.py` (old) does exactly this wrong thing; do not imitate it.

Every mode writes OUT/{adapter_model.safetensors, adapter_config.json} with
r=32, lora_alpha=32, use_rslora=False (=> scaling 1.0 => B_new@A_new == merged dW)
and passes offline/deploy_check.py.

Modes:
  soup       ADIRS...                         uniform product-SVD soup        (B7-1, exp58)
  wsoup      --weights w,w,.. ADIRS...         weighted product-SVD soup      (B7-8)
  wiseft     --alpha a A B                     pairwise interp [1-a, a]       (B7-3, exp62)
  scale      --s S ADIR                        mul every lora_B by S (no SVD) (B7-4, exp63)
  ties       --density D ADIRS...              elementwise trim/sign/disjoint (B7-5, exp66)
  dare       --rate P --density D ADIRS...     DARE drop+rescale then TIES    (B7-5, exp66)
  graft      --spec spec.json ADIRS...         per-module-group source select (B7-6, exp65)
  modelstock ADIRS... [--anchor zero]          closed-form center-by-angle    (B7-11, exp59)
  seedlora   --sigma Q --substage ties ADIRS.. two-stage robust/conflict+SVD  (B7-12, exp61)
  lego       --k 32 ADIRS...                   MSU rank-wise k-means          (B7-13, exp60)
  normeq     ADIRS...                          DisTaC norm-equalize -> soup   (B7-14, exp64)
  knots      --substage ties ADIRS...          joint-SVD align -> TIES        (B7-17, exp69)
  corespace  --substage ties ADIRS...          LoRA-native core merge         (B7-18, exp70)
...
def main(argv=None):
    ...
    ap.add_argument("--substage", default="ties")
    ap.add_argument("--anchor", default="zero")
    ...
    m = a.mode
    if m == "soup":
        soup(a.adirs, out=a.out)
    elif m == "wsoup":
        w = [float(x) for x in a.weights.split(",")]
        soup(a.adirs, weights=w, out=a.out)
    elif m == "wiseft":
        wiseft(a.alpha, a.adirs[0], a.adirs[1], out=a.out)
    elif m == "scale":
        scale(a.s, a.adirs[0], out=a.out)
    elif m == "ties":
        ties(a.adirs, a.density, out=a.out)
    elif m == "dare":
        dare(a.adirs, a.rate, a.density, out=a.out)
    elif m == "graft":
        graft(a.adirs, json.load(open(a.spec)), out=a.out)
    elif m == "modelstock":
        modelstock(a.adirs, anchor=a.anchor, out=a.out)
    elif m == "seedlora":
        seedlora(a.adirs, a.sigma, a.substage, out=a.out)
    elif m == "lego":
        lego(a.adirs, a.k, out=a.out)
    elif m == "normeq":
        normeq(a.adirs, out=a.out)
    elif m == "knots":
        knots(a.adirs, a.substage, a.density, out=a.out)
    elif m == "corespace":
        corespace(a.adirs, a.substage, a.density, out=a.out)
    elif m == "diag":
        diag(a.diag, a.adirs[0] if a.adirs else a.anchor, a.sources)
    else:
