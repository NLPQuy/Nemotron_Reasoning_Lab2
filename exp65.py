# %% [markdown]
# # Batch-7 LoRA fusion (ZERO-TRAIN) — exp65
# Self-contained Kaggle CPU notebook: attach the 5x0.86 adapter dataset, Run All -> submission.zip.
# >>> EXP65 START
# EXP65 — B7-6 graft — per-module-group source selection (Frankenmerge, no SVD).
# Mode: graft(auto_pool(), SPEC): copy native rank-32 A,B per module group from the chosen adapter.
# Note: Round-robins discovered adapters across module groups; edit SPEC to explore.
...
POOL = auto_pool()
groups = ["attn", "in_out_proj", "experts", "lm_head"]
SPEC = {g: POOL[i % len(POOL)] for i, g in enumerate(groups)}  # round-robin source per group
graft(POOL, SPEC, out=OUT)
# >>> EXP65 END
...
deploy_check(OUT)
zip_submission(OUT)
print("exp65 done: /kaggle/working/submission.zip")
