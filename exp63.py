# %% [markdown]
# # Batch-7 LoRA fusion (ZERO-TRAIN) — exp63
# Self-contained Kaggle CPU notebook: attach the 5x0.86 adapter dataset, Run All -> submission.zip.
# >>> EXP63 START
# EXP63 — B7-4 scale — rescale one adapter's delta by a global factor (no SVD).
# Mode: scale(S, auto_pool()[0]): multiply every lora_B by S (NOT base_layer.weight).
# Note: Sweep S in {0.9,0.95,1.05,1.1}; diag drift tracks S.
...
ADIR = auto_pool()[0]  # scale the first discovered adapter (reorder to pick another)
S = 1.05  # sweep {0.9,0.95,1.05,1.1} across Kaggle runs
scale(S, ADIR, out=OUT)
# >>> EXP63 END
...
deploy_check(OUT)
zip_submission(OUT)
print("exp63 done: /kaggle/working/submission.zip")
