# %% [markdown]
# # Batch-7 LoRA fusion (ZERO-TRAIN) — exp64
# Self-contained Kaggle CPU notebook: attach the 5x0.86 adapter dataset, Run All -> submission.zip.
# >>> EXP64 START
# EXP64 — B7-14 DisTaC norm-equalize — equalize per-module delta norms, then soup.
# Mode: normeq(auto_pool()): rescale each adapter's ||dW||_F to the per-module median, then uniform soup.
# Note: Pre-step so no single mis-scaled adapter dominates the average.
...
deploy_check(OUT)
zip_submission(OUT)
print("exp64 done: /kaggle/working/submission.zip")
