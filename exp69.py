# %% [markdown]
# # Batch-7 LoRA fusion (ZERO-TRAIN) — exp69
# Self-contained Kaggle CPU notebook: attach the 5x0.86 adapter dataset, Run All -> submission.zip.
# >>> EXP69 START
# EXP69 — B7-17 KnOTS — joint-SVD align all deltas to a shared basis, then TIES.
# Mode: knots(auto_pool(), substage=ties): concat-across-in joint SVD -> shared U -> TIES on aligned coords.
# Note: ⭐ submit candidate; resolves misaligned factorization before sign-election.
...
POOL = auto_pool()
knots(POOL, substage="ties", density=0.7, out=OUT)
# >>> EXP69 END
...
deploy_check(OUT)
zip_submission(OUT)
print("exp69 done: /kaggle/working/submission.zip")
