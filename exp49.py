...
# >>> EXP49 START
# EXP49 — Flat-LoRA (Batch-6 B6-7): flat-minimum cho greedy decode.
# Ref: refs/flat-lora/logTrainer.py:148-186 (perturb base weight, restore same noise, 1 fwd).
# Knob: FLAT_RHO=0.05 (dò {0.0,0.02,0.05,0.1}) | Rollback: FLAT_RHO=0.0
# Deploy: base được khôi phục trước save -> adapter sạch rank-32 chuẩn.
# >>> EXP49 END
...
BATCH_SIZE = 32
MICRO_BATCH_SIZE = 4
# >>> EXP49_CONT START   (continue-train từ 0.86 — luật batch-6)
...
def run_training() -> None:
    ...
    # >>> EXP49 START — B6-7 Flat-LoRA (LoRA-factor space, memory-frugal). Rollback: FLAT_RHO=0.0
    # NOTE: original Flat-LoRA perturbs the MERGED weight W0+s·BA and stores a full base-weight-sized
    # noise per module → effectively a 2nd copy of the model → OOM on 30B/94GB. Here we perturb the
    # trainable LoRA factors A/B directly (rank-32, tiny noise) — SAM-style sharpness perturbation in
    # the adapter subspace. Memory-light; restored exactly via the small stored noise.
    FLAT_RHO = 0.05  # dò {0.0(off), 0.02, 0.05, 0.1}
    _flat_mods = [
        _m for _m in model.modules()
        if hasattr(_m, "lora_A") and hasattr(_m, "base_layer")
        and "default" in getattr(_m, "lora_A", {})
    ]
    _flat_saved = []  # (param_tensor, noise) to restore exactly
    ...
    def _flat_perturb(progress: float) -> None:
        ...
        with torch.no_grad():
            for _m in _flat_mods:
                for _w in (_m.lora_A["default"].weight, _m.lora_B["default"].weight):
                    _rms = _w.norm() / max(1.0, float(_w.numel()) ** 0.5)  # relative scale
                    _std = factor * FLAT_RHO * _rms
                    if float(_std) <= 0:
                        continue
                    _noise = torch.randn_like(_w) * _std
                    _w.add_(_noise)
                    _flat_saved.append((_w, _noise))

    def _flat_restore() -> None:
        with torch.no_grad():
            for _w, _noise in _flat_saved:
                _w.sub_(_noise)
        _flat_saved.clear()
    # >>> EXP49 END
    # ── Training loop ────────────────────────────────────────────────
