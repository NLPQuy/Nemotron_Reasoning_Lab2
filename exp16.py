    ...
    for batch_start in range(0, len(indices), BATCH_SIZE):
        ...
        for mb_start in range(0, n, MICRO_BATCH_SIZE):
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                ...
                # >>> EXP16 START
                if KL_ANCHOR_BETA > 0 and weight_sum_t > 0:
                    # CCE path intentionally avoids materializing logits. For KL we
                    # pay one extra adapted forward plus one adapter-disabled base
                    # forward on completion tokens. If this OOMs, replace the full
                    # logits below with a top-k approximation over masked positions.
                    with model.disable_adapter(), torch.no_grad():
                        ...
                        base_logits = _base.lm_head(base_hidden)
                    pol_hidden = _base.backbone(
                        input_ids=padded_input,
                        attention_mask=attention_mask,
                        use_cache=False,
                    )[0]
                    pol_logits = _base.lm_head(pol_hidden)
                    m = padded_weights > 0
                    kl = torch.nn.functional.kl_div(
                        torch.log_softmax(pol_logits[m], dim=-1),
                        torch.log_softmax(base_logits[m], dim=-1),
                        log_target=True,
                        reduction="batchmean",
                    )
