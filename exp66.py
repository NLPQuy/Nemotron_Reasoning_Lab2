# %% [markdown]
# # Batch-7 LoRA fusion (ZERO-TRAIN) — exp66
# Self-contained Kaggle CPU notebook: attach the 5x0.86 adapter dataset, Run All -> submission.zip.
# >>> EXP66 START
# EXP66 — B7-5 TIES / DARE-SVD — elementwise trim/sign/disjoint merge then SVD-32.
# Mode: ties(auto_pool(), density=0.7) (DARE alt: dare(..., rate=0.5, density=0.7)).
# Note: Only worth submitting if exp58 >= 0.86 (interference-resolution on top of soup).
...
POOL = auto_pool()
ties(POOL, density=0.7, out=OUT)
# DARE-TIES alternative:
# dare(POOL, rate=0.5, density=0.7, out=OUT)
# >>> EXP66 END
...
deploy_check(OUT)
zip_submission(OUT)
print("exp66 done: /kaggle/working/submission.zip")
